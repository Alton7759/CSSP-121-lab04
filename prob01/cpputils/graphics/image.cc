...
bool Image::DrawCircle(int x, int y, int radius, int red, int green, int blue) {
  const int color[] = {red, green, blue};
  if (!CheckPixelInBounds(x, y) || !CheckColorInBounds(color) || !CheckRadiusInBounds(radius)) {
    return false;
  }
...
bool Image::CheckRadiusInBounds(int radius) const {
  if (radius < 0) {
    cout << "Invalid size: " << radius << endl;
    return false;
  }
  return true;
}
